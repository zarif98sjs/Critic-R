...
def evaluate_documents(query: str, instruction: str, documents: str, feedback: str) -> Tuple[bool, str, str]:
    """Evaluate if documents match the Reasoning Model's feedback."""
    eval_prompt = f"""Evaluate the search results based on the User's Reasoning Feedback.

Current Search Instruction: {instruction}
Current Query: {query}

Retrieved Documents:
{documents}

User's Reasoning Feedback (Critique of these documents):
{feedback}

Task:
1. Does the feedback indicate the documents are useful and sufficient? 
2. Does the feedback indicate the search was a failure or missing information?

Output Format:
If satisfactory: <satisfactory>yes</satisfactory>
If NOT satisfactory:
<satisfactory>no</satisfactory>
<instruction>refined instruction</instruction>
...
"""
    ...
    new_query = extract_tag_content(response, "query") or query
...
def refine_retrieval(context_ids: torch.Tensor, original_query: str) -> Tuple[str, List[Dict], List[Dict]]:
    ...
    for attempt in range(MAX_REFINEMENT_TRIES + 1):
        ...
        is_satisfactory, new_instruction, new_query, eval_log_data = evaluate_documents(
            current_query, current_instruction, current_documents, updated_feedback
        )
...
If you find no further external knowledge needed, you can directly provide the answer inside <answer> and </answer>, without detailed illustrations. For example, <answer> Beijing </answer>. Question: {question}\n"""
    ...
    for sample_idx in range(NUM_SAMPLES_PER_QUESTION):
        ...
        while True:
            ...
            if search_query:
                ...
                current_documents = search_documents(search_query, instruction="Given a query, retrieve relevant passages that answer the query.")
                ...
                doc_block = f"\n<information>{current_documents}</information>\n"
    ...
    return is_correct_at_least_once
...
if __name__ == "__main__":
    ...
    LOG_FILE_PATH = f'SIGSHORTv2/inference_log__{DATASET_NAME}__{REASONING_MODEL_ID__}__{EVALUATOR_MODEL_ID__}__{RETRIEVAL_MODEL_ID__}__{args.start_idx}__{args.end_idx}.jsonl'
    ...
    with open(LOG_FILE_PATH, 'w', encoding='utf-8') as log_file:
        for idx, item in enumerate(tqdm(dataset[SPLIT])):
            ...
            is_correct_at_least_once = process_question(question, ground_truths, idx, log_file)
