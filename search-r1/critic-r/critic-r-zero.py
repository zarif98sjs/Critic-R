...
async def evaluate_documents(og_query: str, sub_query: str, instruction: str, documents: str, feedback: str, session: aiohttp.ClientSession) -> Tuple[bool, str, str, Dict]:
    """Evaluate if documents match the Reasoning Model's feedback, splitting the cognitive load."""
    
    # STEP 1: Judge satisfaction and provide a reason
    eval_prompt_1 = f"""You are an evaluator for a search problem. You will be given a global search query, a local sub-query, retrieved documents, and critique feedback from a reasoning model. Your ONLY task is to evaluate if the retrieved documents are satisfactory for answering the **current sub-query**.
...
# OUTPUTS:
    - satisfactory: If documents are sufficient to answer the current sub-query, output: <satisfactory>yes</satisfactory>. Otherwise output: <satisfactory>no</satisfactory>.
    - reason: Provide a concise reason for your decision. If 'no', state exactly what is missing. Output format: <reason>reason here</reason>
"""

    payload_1 = {
        "model": EVALUATOR_MODEL_ID,
        "messages": [{"role": "user", "content": eval_prompt_1}],
        "temperature": 0.0,
        "max_tokens": 512
    }

    try:
        async with session.post(EVALUATOR_ENDPOINT, json=payload_1) as api_response:
            api_response.raise_for_status()
            response_data = await api_response.json()
            response_text_1 = response_data['choices'][0]['message']['content']
    except Exception as e:
        print(f"Evaluator API Error (Step 1): {e}")
        response_text_1 = ""

    is_satisfactory = "yes" in (extract_tag_content(response_text_1, "satisfactory") or "").lower()
    reason = extract_tag_content(response_text_1, "reason") or "No reason provided."

    # Default to keeping the current parameters
    new_instruction = instruction
    new_query = sub_query
    response_text_2 = None
    eval_prompt_2 = None

    # STEP 2: Only if unsatisfactory, generate new search parameters
    if not is_satisfactory:
        eval_prompt_2 = f"""You are a search query optimizer. The previous search failed to retrieve satisfactory documents. 
...
# OUTPUTS:
    - new instruction: A concise, refined instruction for the next retrieval attempt. Output format: <instruction>refined instruction</instruction>
    - new query: A concise, refined query. Output format: <query>refined query</query>
"""
        payload_2 = {
            "model": EVALUATOR_MODEL_ID,
            "messages": [{"role": "user", "content": eval_prompt_2}],
            "temperature": 0.0,
            "max_tokens": 512
        }

        try:
            async with session.post(EVALUATOR_ENDPOINT, json=payload_2) as api_response:
                api_response.raise_for_status()
                response_data = await api_response.json()
                response_text_2 = response_data['choices'][0]['message']['content']
                
                # Extract the new parameters
                new_instruction = extract_tag_content(response_text_2, "instruction") or instruction
                new_query = extract_tag_content(response_text_2, "query") or sub_query
                
        except Exception as e:
            print(f"Evaluator API Error (Step 2): {e}")

    eval_log = {
        "evaluation_prompt_1": eval_prompt_1,
        "evaluator_response_1": response_text_1,
        "evaluation_prompt_2": eval_prompt_2,
        "evaluator_response_2": response_text_2,
...
if __name__ == "__main__":
    ...
    correct_answer_file_path = f'SIGSHORTv2/correct_answer_samples__opt2v2__{DATASET_NAME}__{REASONING_MODEL_ID__}__{EVALUATOR_MODEL_ID__}__{RETRIEVAL_MODEL_ID__}__{args.start_idx}__{args.end_idx}.jsonl'
    LOG_FILE_PATH = f'SIGSHORTv2/inference_time_scaling_log__opt2v2__{DATASET_NAME}__{REASONING_MODEL_ID__}__{EVALUATOR_MODEL_ID__}__{RETRIEVAL_MODEL_ID__}__{args.start_idx}__{args.end_idx}.jsonl'
